...
async function loadSelectedServerOnModsTab(){
    ...
    for(const el of document.getElementsByClassName('settingsSelServContent')) {
        const iconSrc = (typeof serv.rawServer.icon === 'string' && serv.rawServer.icon.trim().length > 0 && serv.rawServer.icon !== 'null') ? serv.rawServer.icon : './assets/images/Icon.png'
        el.innerHTML = `
            <img class="serverListingImg" src="${iconSrc}"/>
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/rtamc/HAN-Launcher/releases.atom',
        success: (data) => {
